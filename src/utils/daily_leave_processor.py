...
def process_leaves(target_date: str, test_mode: bool = False, send_line: bool = False) -> str:
    ...
    # Note: Teacher hours snapshot is now written by webhook after admin finalization
    # This ensures snapshot reflects the finalized assignments in Leave_Logs
    if not test_mode:
        print(f"\nNote: Teacher hours snapshot will be updated after admin finalization via webhook")
