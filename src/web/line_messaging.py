...
def send_to_admin_group(message: str) -> bool:
    """
    ...
    Returns:
        True if successful, False otherwise
    """
    # Use admin group ID, fallback to legacy LINE_GROUP_ID
    admin_group = config.LINE_ADMIN_GROUP_ID or config.LINE_GROUP_ID

    if not admin_group:
        print("ERROR: LINE_ADMIN_GROUP_ID not set")
        return False

    return send_message_to_group(message, admin_group)


def send_to_teacher_group(message: str) -> bool:
    """
    Send a message to the teacher group.
    Note: In the two-group system, admins manually copy and send reports.
    This function is available for backward compatibility or testing.
    ...
    Returns:
        True if successful, False otherwise
    """
    # Use teacher group ID, fallback to legacy LINE_GROUP_ID
    teacher_group = config.LINE_TEACHER_GROUP_ID or config.LINE_GROUP_ID

    if not teacher_group:
        print("ERROR: LINE_TEACHER_GROUP_ID not set")
        return False

    return send_message_to_group(message, teacher_group)


def send_daily_report(balloon1: str, balloon2: str = None) -> bool:
    """
    Send daily substitute teacher report to admin group in two separate balloons.
    Admins will review and manually forward to teacher group.

    Args:
        balloon1: Main report with [REPORT] prefix and substitute assignments
        balloon2: Admin instructions (optional, sent as second message if provided)

    Returns:
        True if all messages sent successfully, False otherwise
    """
    # Send balloon 1 (main report)
    success1 = send_to_admin_group(balloon1)

    if not success1:
        return False

    # Send balloon 2 (admin instructions) if provided
    if balloon2:
        # Small delay between messages for better readability
        import time
        time.sleep(0.5)
        success2 = send_to_admin_group(balloon2)
        return success2
...
def send_error_notification(error_message: str) -> bool:
    ...
    Returns:
        True if successful, False otherwise
    """
    formatted_message = f"⚠️ เกิดข้อผิดพลาดในระบบ\n\n{error_message}"

    return send_to_admin_group(formatted_message)
...
def send_test_message() -> bool:
    ...
    test_message = (
        "🧪 ทดสอบระบบ LINE Bot\n\n"
        "ระบบจัดการครูแทนทำงานปกติ\n"
        f"เวลาประมวลผลรายวัน: {config.DAILY_PROCESS_TIME} น.\n\n"
        "หากได้รับข้อความนี้ แสดงว่าการตั้งค่า LINE Bot สำเร็จ ✓"
    )
...
def format_substitute_summary(
    ...
    message = (
        f"📅 {date}\n\n"
        f"👥 ครูที่ลา: {total_absences} คน\n"
        f"📚 คาบที่ต้องจัดครู: {total_periods} คาบ\n"
        f"✅ จัดครูแทนสำเร็จ: {substitutes_found}/{total_periods} "
        f"({success_rate:.0f}%)\n"
    )

    if substitutes_found < total_periods:
        missing = total_periods - substitutes_found
        message += f"\n⚠️ หาครูแทนไม่ได้: {missing} คาบ"
...
def format_detailed_assignment(
    ...
    if substitute_teacher:
        return (
            f"{day} คาบ {period} | {class_id} - {subject}\n"
            f"  {absent_teacher} → {substitute_teacher}"
        )
    else:
        return (
            f"{day} คาบ {period} | {class_id} - {subject}\n"
            f"  {absent_teacher} → ❌ ไม่มีครูแทน"
        )
...
def send_formatted_report(
    ...
    message_lines.append(f"👥 ครูที่ลา: {total_absences} คน")
    message_lines.append(f"📚 จำนวนคาบ: {total_periods} คาบ")
    message_lines.append(f"✅ หาครูแทนได้: {substitutes_found}/{total_periods} คาบ")
    ...
    for day in sorted(by_day.keys()):
        ...
        for assignment in day_assignments:
            ...
            substitute = assignment.get('substitute_teacher', 'ไม่มีครูแทน')

            if substitute and substitute != 'ไม่มีครูแทน':
                status = "✅"
            else:
                status = "❌"
                substitute = "ไม่มีครูแทน"

            message_lines.append(
                f"{status} คาบ {period} | {class_id} - {subject}\n"
                f"   {absent} → {substitute}"
            )
    ...
    message_lines.append("🤖 สร้างโดยระบบอัตโนมัติ")
